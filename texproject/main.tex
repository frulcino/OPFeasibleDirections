...
\gr[]{forse ho sbagliato qualcos apechè estendendo puunti di minimo non sunziona}
\begin{align}
	{b_n} \leq a_n\inf_{x \in C_a^+}\frac{\pi^{n-1}(x) - p(a)}{p(x)-p(a)} (\geq 0)\\
	{b_n} \geq a_n\sup_{x \in C_a^-}\frac{\pi^{n-1}(x) - p(a)}{p(x)-p(a)} (\leq 0) \\
	0 \leq \inf_{x \in C_a^0}\pi^{n-1}(x) - p(a) (\geq 0) \label{eq: 3}
\end{align}
We note that equation \eqref{eq: 3} is always satisfied. \gr[]{this is not a big help because we still need to check a lot of points rip}
