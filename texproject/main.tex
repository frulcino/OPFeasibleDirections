...
\begin{document}
	...
	\begin{align}
		\sum_{I \in \cI_j}c_I^j\prod_{v \in I} x_v  \leq b_j \quad \forall j \in \{1,2,\ldots, m\} \\
	...
	\begin{align}
		...
		x_v \in [l_v,u_v] \quad \forall v \in V
	\end{align}
	\gr[inline]{What follows is was our first try (the one we coded), it took too long to be meaningfully tested and made the problem a MILP. We are now trying to keep it LP.}
...
\begin{proposition}
	...
	% Again, by Proposition~\ref{prop: sufficient and necessary sep hyperplane}, it suffices to verify that for all \(J \subset I\) and \(j \in J\):
	% \[
	% \left(\prod_{\substack{v \in I \\ v \neq j}}a_v - \prod_{\substack{v \in I \\ v \neq j}}x^{J}_v\right)(\aop_j - a_j) \geq 0.
	% \]


	% \item If \(\prod_{\substack{v \in I \\ v \neq j}}a_v \geq 0\), then \(a_j \leq 0\) and \(a_j \geq \aop_j\). Since 
	% \[
	% a_j\prod_{\substack{v \in I \\ v \neq j}}a_v \geq a_j\prod_{\substack{v \in I \\ v \neq j}}x^{J}_v,
	% \]
	% it follows that 
	...
	%  If \(\prod_{\substack{v \in I \\ v \neq j}}a_v \leq 0\), then \(a_j \geq 0\) and \(a_j \leq \aop_j\). Since
	% \[
	% a_j\prod_{\substack{v \in I \\ v \neq j}}a_v \geq a_j\prod_{\substack{v \in I \\ v \neq j}}x^{J}_v,
	% \]
	% we have
...
In particular, if both \(C_a^+\cap \delta(a)\) and \(C_a^-\cap \delta(a)\) (where \(\delta(a) \) are the neighbouring verticeso of \(a\)) are not empty, that is, if \(a\) si neither a local maximum, nor  a local minimum,
 ...
 \end{proposition}
\gr[inline]{i minimi (massimi locali) sono dati da quei punti aventi per ciascuna cordinata per cui gli estremi dell'intervallo ha lo stesso segno, dall'estremo col modulo maggiore. Mentre per ciascuno intervallo con estremi con segni discordi una qualunque scelta in modo tale che ne ha un numero dispari di segni negativi se è un minimo locale e un numero pari se è massimo locale.}

\gr[]{questo vuol dire che se tutte gli intervalli sono positivi gli unici hyperplane che separano sono quelli dei massimi e minimi globali?}
