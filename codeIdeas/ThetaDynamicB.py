...
def Solve(case,plot,theta_lim_):
    ...
    for i in range(len(case["bus"])):
        bus_i = int(case["bus"][i]["bus_i"])
        theta[bus_i] = model.addVar(lb=-theta_lim_, ub=theta_lim_, name=f"theta {bus_i}")
        theta_lim[bus_i]=[-theta_lim_,theta_lim_]
    ...
    for i in range(len(case['branch'])):
        ...
        c[i]=model.addVar(lb=math.cos(2*theta_lim_),ub=1,name=f"c{(fbus,tbus,i)}")
        s[i]=model.addVar(lb=-math.sin(2*theta_lim_),ub=math.sin(2*theta_lim_),name=f"s{(fbus,tbus,i)}")
        c_lim[i]=[math.cos(2*theta_lim_),1]
        s_lim[i]=[-math.sin(2*theta_lim_),math.sin(2*theta_lim_)]
        c_mult[i]=model.addVar(lb=vm_min*vk_min*math.cos(2*theta_lim_),ub=vm_max*vk_max,name=f"c_mult{(fbus,tbus,i)}") #jabr c_km
        s_mult[i]=model.addVar(lb=-vm_max*vk_max*math.sin(2*theta_lim_),ub=vm_max*vk_max*math.sin(2*theta_lim_),name=f"s_mult{(fbus,tbus,i)}") #jabr s_km

    
    gap=abs(math.sin(2*theta_lim_)-2*theta_lim_)
    nb_step=20
    A={}
    ...
    def score(s_lim,c_lim,v_lim,theta_lim):
        score_=0
        for j in s_lim:
            i=s_lim[j]
            score_+=i[1]-i[0]
    ...
    Bound_constr=[]
    Bound_score=[]
    Bound_score.append(score(s_lim,c_lim,v_lim,theta_lim))
    bound_cond=0
    while bound_cond<2 and Temp_iter<30:
        if bound_cond==1:
            bound_cond+=1
        T=0
        r=0
        ...
        ##Supprimer les bounds précédentes
        for constr in Bound_constr:
            model.remove(constr)

        ###Mettre c_lim,s_lim proche de la solution optimale d'un certain facteur de température.
        mem_s_lim=s_lim.copy()
        mem_c_lim=c_lim.copy()
        mem_vlim=v_lim.copy()
        mem_theta_lim=theta_lim.copy()
        reduc_temp=math.exp(-Temp_iter/2)
        for j in mem_s_lim:
            i=mem_s_lim[j]
            length=i[1]-i[0]
            s_lim[j]=[s[j].X-length/2*reduc_temp,s[j].X+length/2*reduc_temp]
        for j in mem_c_lim:
            i=mem_c_lim[j]
            length=i[1]-i[0]
            c_lim[j]=[c[j].X-length/2*reduc_temp,c[j].X+length/2*reduc_temp]
        for j in v_lim:
            i=mem_vlim[j]
            length=i[1]-i[0]
            v_lim[j]=[v[j].X-length/2*reduc_temp,v[j].X+length/2*reduc_temp]
        for j in theta_lim:
            i=mem_theta_lim[j]
            length=i[1]-i[0]
            theta_lim[j]=[theta[j].X-length/2*reduc_temp,theta[j].X+length/2*reduc_temp]
            
        ### Puis, calculer les bornes

        for j in mem_s_lim:
            Bound_constr.append(model.addConstr(s[j]>=s_lim[j][0]))
            Bound_constr.append(model.addConstr(s[j]<=s_lim[j][1]))
        for j in mem_c_lim:
            Bound_constr.append(model.addConstr(c[j]>=c_lim[j][0]))
            Bound_constr.append(model.addConstr(c[j]<=c_lim[j][1]))
        for j in v_lim:
            Bound_constr.append(model.addConstr(v[j]>=v_lim[j][0]))
            Bound_constr.append(model.addConstr(v[j]<=v_lim[j][1]))
        for j in theta_lim:
            Bound_constr.append(model.addConstr(theta[j]>=theta_lim[j][0]))
            Bound_constr.append(model.addConstr(theta[j]<=theta_lim[j][1]))
        ...
        for i in tqdm(range(len(case['branch'])),ncols=100,desc=f"Linear cos and sine"): ### Linear cos and sine
            fbus=int(case['branch'][i]['fbus'])
            tbus=int(case['branch'][i]['tbus'])
            gap=abs(math.sin(theta_lim[fbus][1]-theta_lim[tbus][0])-(theta_lim[fbus][1]-theta_lim[tbus][0]))
            # model.addConstr(s[i]==(theta[fbus]-theta[tbus]))
            Bound_constr.append(model.addConstr(s[i]<=(theta[fbus]-theta[tbus])+gap))
            Bound_constr.append(model.addConstr(s[i]>=(theta[fbus]-theta[tbus])-gap))
            print("GAP : "+str(gap))

            max_theta=theta_lim[fbus][1]-theta_lim[tbus][0]
            min_theta=theta_lim[fbus][0]-theta_lim[tbus][1]
            if max_theta-min_theta<1e-8:
                Bound_constr.append(model.addConstr(c[i]==math.cos(min_theta)))
            else:   
                a=(math.cos(max_theta)-math.cos(min_theta))/(max_theta-min_theta)
                b=-min_theta*a+math.cos(min_theta)
                Bound_constr.append(model.addConstr(c[i]>=a*(theta[fbus]-theta[tbus])+b))

            pos_init=theta_lim[fbus][0]-theta_lim[tbus][1]
            for k in range(0,nb_step):
                x_bar=pos_init+k/nb_step*gap
                a=-math.sin(x_bar)
                b=math.cos(x_bar)-x_bar*a
                Bound_constr.append(model.addConstr(c[i]<=a*(theta[fbus]-theta[tbus])+b))
        ...
        for i in tqdm(range(len(case["bus"])),ncols=100,desc="Linear v->v²"):
            ...
            vk_min=v_lim[fbus][0]
            vk_max=v_lim[fbus][1]
            if vk_max-vk_min<1e-8:
                Bound_constr.append(model.addConstr(vk_max**2 == v2[fbus],name=f"v2 {fbus}"))
            else:
                a=(vk_max+vk_min)
                b=-vk_min*a+vk_min**2
