...
def Solve(case,plot,theta_lim):
    ...
    gap=abs(math.sin(2*theta_lim)-2*theta_lim)
    nb_step=20
    A={}
    ...
    gap_factor=1/10
    while True:
        gap_factor=gap_factor*1/10
        print(f"GAP FACTOR = {gap_factor}")

        for constr in Bound_constr:
            model.remove(constr)
        mem_bounds=[v_lim.copy()]
        for i in v:
            Bound_constr.append(model.addConstr(v[i]<=v[i].X+gap_factor,name="equal v"))
            Bound_constr.append(model.addConstr(v[i]>=v[i].X-gap_factor,name="equal v"))
            v_lim[i]=[v[i].X-gap_factor,v[i].X+gap_factor]
        nb_step=5
        for i in tqdm(range(len(case["bus"])),ncols=100,desc="Linear v->v²"):
            fbus=case["bus"][i]["bus_i"]
            if v_lim[fbus][1]-v_lim[fbus][0]<1e-5:
                Bound_constr.append(model.addConstr(v2[fbus]==v[fbus].X**2,name="v2²²²"))
            else:
                a=(vk_max+vk_min)
                b=-vk_min*a+vk_min**2
        ...
        for i in tqdm(range(len(case['branch'])),ncols=100,desc="Linear Mult cv² / sv²"): ### Linear mult
            ...
            tbus=int(case['branch'][i]['tbus'])
            if v_lim[fbus][1]-v_lim[fbus][0]<1e-5:
                Bound_constr.append(model.addConstr(c_mult[i]==v[fbus].X*v[tbus].X*c[i],name="C EXACT²²²"))
                Bound_constr.append(model.addConstr(s_mult[i]==v[fbus].X*v[tbus].X*s[i],name="S EXACT²²²"))
            else:
                vars=[c[i],v[fbus],v[tbus]]
                cuboid=[c_lim[i],v_lim[fbus],v_lim[tbus]]
                HP=hyperplanes.generate_all_neighbor_hp(cuboid)
                for hp in HP : #COMPUTE HYPERPLANES INEQUALITIES
                    ...
                    constraint=np.dot(hyperplane,vars)+constant
                    if hp[1]>0:
                        Bound_constr.append(model.addConstr(constraint >= c_mult[i],name=f"equal c_mult {fbus} {tbus}"))
                        # print(f"HP, {loop_number,k,id_comb}")
                    elif hp[1]<0:
                        Bound_constr.append(model.addConstr(constraint <= c_mult[i],name=f"equal c_mult {fbus} {tbus}"))
                        # print(f"HP, {loop_number,k,id_comb}")
                ...
                for hp in HP : #COMPUTE HYPERPLANES INEQUALITIES
                    ...
                    constraint=np.dot(hyperplane,vars)+constant
                    if hp[1]>0:
                        Bound_constr.append(model.addConstr(constraint >= s_mult[i],name=f"equal s_mult {fbus} {tbus}"))
                        # print(f"HP, {loop_number,k,id_comb}")
                    elif hp[1]<0:
                        Bound_constr.append(model.addConstr(constraint <= s_mult[i],name=f"equal s_mult {fbus} {tbus}"))
                        # print(f"HP, {loop_number,k,id_comb}")
        ...
        obj=model.getObjective()
        val.append(obj.getValue())
        nb_const_tot.append(0)
        nb_const_deleted.append(0)
