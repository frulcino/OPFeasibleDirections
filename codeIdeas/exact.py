...
def Solve(case,plot):
    ...
    Theta={}
    for i in range(len(case["bus"])):
        index=case["bus"][i]["bus_i"]
        Theta[index]=model.addVar(lb=-math.pi/2,ub=math.pi/2,name=f"Theta {index}")
        Theta[index].start=0
    ...
    for i in range(len(case['branch'])):
        ...
        # theta_=Theta[fbus]-Theta[tbus]
        # model.addConstr(c[i]==v[fbus]*v[tbus]*(1-(theta_**2)/2+(theta_**4)/12-(theta_**6)/720),name=f"Cos {fbus,tbus}")
        # model.addConstr(s[i]==v[fbus]*v[tbus]*(theta_-(theta_**3)/6+(theta_**5)/60),name=f"Sin {fbus,tbus}")
    ...
    # for loop in loops:
    #     tot_mod_2pi = model.addVar(vtype="I", name=f"tot_mod_2pi {loops.index(loop)}")
    #     for i in loop:
    #         tot_theta=0
    #         tot_theta+=Theta[i]
    #     model.addConstr(tot_mod_2pi==tot_theta/(2*math.pi))
    ...
    for i in tqdm(range(len(case['branch'])), desc="Const. (6) Jabr ..." ,ncols=100):
        ...
        model.addConstr(P[i]/baseMVA==G[(k,k)]*v[k]**2+G[(k,m)]*c[i]+B[(k,m)]*s[i], name=f"6a_{k},{m},{i}")
        model.addConstr(P[i+N]/baseMVA==G[(m,m)]*v[m]**2+G[(m,k)]*c[i]-B[(m,k)]*s[i], name=f"6b_{m},{k},{i}")
        model.addConstr(Q[i]/baseMVA==-B[(k,k)]*v[k]**2-B[(k,m)]*c[i]+G[(k,m)]*s[i], name=f"6c_{k},{m},{i}")
        model.addConstr(Q[i+N]/baseMVA==-B[(m,m)]*v[m]**2-B[(m,k)]*c[i]-G[(m,k)]*s[i], name=f"6d_{m},{k},{i}")

    for i in range(len(case["branch"])): #SCOP Jabr
        fbus=case["branch"][i]["fbus"]
        tbus=case["branch"][i]["tbus"]
        aux_var = model.addVar(name=f"aux_{i}")
        model.addConstr(aux_var == v[fbus] * v[tbus])
        model.addConstr(c[i] ** 2 + s[i] ** 2 == aux_var ** 2)
    ...
    for i in range(len(case['branch'])):
        ...
        i2like=Alpha*v[fbus]**2+Beta*v[tbus]**2+Gamma*c[i]+Ki*s[i]
    ...
    def JabrCuts(eps,pT,Const_to_add): 
        ...
        for i in range(len(case['branch'])):
            fbus=case['branch'][i]["fbus"]
            tbus=case['branch'][i]["tbus"]
            amount=c[i].X*c[i].X+s[i].X*s[i].X-v[fbus].X**2*v[tbus].X**2
            if amount>eps:
                violation_amount.append([amount,i,c[i].X,s[i].X,v[fbus].X**2,v[tbus].X]**2)
    ...
    # for i in range(len(violation_amount)):
        for i in range(len(violation_amount)):
            ...
            if v2k+v2m>eps:
                ...
                Const_to_add.append([4*c_*c[branch_i]+4*s_*s[branch_i]+(v2k-v2m-n0)*v[fbus]**2+(v2m-v2k-n0)*v[tbus]**2,f"CutJabr_{fbus},{tbus},{branch_i}"])
    ...
    def i2Cuts(eps,pT,Const_to_add): 
        ...
        for i in range(len(case['branch'])):
            if i in i2:
                fbus=int(case['branch'][i]["fbus"])
                tbus=int(case['branch'][i]["tbus"])
                amount=P[i].X**2+Q[i].X**2-i2[i].X*v[fbus].X**2
                if amount>eps:
                    violation_amount.append([amount,0,P[i].X,Q[i].X,v[fbus].X**2,i2[i].X,i])  
        ...
        for i in range(len(violation_amount)):
            ...
            if v2k+i2kX>eps :
                ...
                Const_to_add.append([4*P_*P[branch_i]+4*Q_*Q[branch_i]+(v2k-i2kX-n0)*v[fbus]**2+(i2kX-v2k-n0)*i2[branch_i],f"Cuti2_{fbus},{tbus},{branch_i}"])
        return len(violation_amount)
    ...
    for i in range(len(case["branch"])):
        ...
        violation=abs(c[i].X**2+s[i].X**2-v[fbus].X**2*v[tbus].X**2)
        if violation<=1e-04:
    ...
    for i in range(len(case['branch'])):
        ...
        val_viol=0
        val_viol+=abs(P[i].X/baseMVA-(G[(k,k)]*v[k].X**2+G[(k,m)]*new_c+B[(k,m)]*new_s))
        val_viol+=abs(P[i+N].X/baseMVA-(G[(m,m)]*v[m].X**2+G[(m,k)]*new_c-B[(m,k)]*new_s))
        val_viol+=abs(Q[i].X/baseMVA-(-B[(k,k)]*v[k].X**2-B[(k,m)]*new_c+G[(k,m)]*new_s))
        val_viol+=abs(Q[i+N].X/baseMVA-(-B[(m,m)]*v[m].X**2-B[(m,k)]*new_c-G[(m,k)]*new_s))
